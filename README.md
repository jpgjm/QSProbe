...
- **バックグラウンドの待ち受けは常時オンではありません。** 既定では、ホームに
  戻ると iOS がプロセスを眠らせるので待ち受けが切れます。トグルをオンにしたまま
  アプリを離れて戻った場合は**自動で張り直し**、アプリを終了して開き直した場合も
  **前回の状態を復元**します。背面でも待ち受け続けたい場合は「画面を閉じても
  待ち受ける」をオンにしてください (下記)。
- **転送中の背面継続は iOS 26 以降だけです。** `BGContinuedProcessingTask` を
  使っています。iOS 25 以前は約 30 秒で打ち切られます。
...
2 つの別々の仕組みを使い分けています。目的が違うので、片方では代用できません。

| 区間 | 仕組み | 既定 | 条件 |
|---|---|---|---|
| 転送の開始〜完了 | `BGContinuedProcessingTask` (iOS 26+) | 常にオン | 前面の操作が起点。進捗が測れる |
| 待ち受け (相手を待つ) | 無音のループ再生 (`UIBackgroundModes: audio`) | **オフ** | ユーザーが明示的にオンにする |

`BGContinuedProcessingTask` は「前面の操作で始まり、進捗が測れて、終わりが
定義できる作業」向けの API です。WWDC25 セッション 227 が明言しているとおり、
**進捗を報告しないタスクは時間切れにされ**、進捗が想定より遅いと続行するか
ユーザーに訊く UI が出ます。相手がいつ送ってくるか分からない待ち受けには
進捗という概念が無いので、この API は使えません。

そこで待ち受けの常駐だけは無音再生に頼っています
(`Sources/BackgroundKeepAlive.swift`)。音を鳴らしているあいだ iOS は
プロセスをサスペンドしないため、`NWListener` と Bonjour の publish が
生きたままになります。`Info.plist` に `audio` を足すだけで、entitlement は
要りません。無料 Personal Team + SideStore で動きます。

背面で受信リクエスト (INTRODUCTION) が届いたときは、ローカル通知で知らせます。
通知に**承諾・拒否のボタンは付けていません**。押せるようにすると確認コードを
見ずに受け入れられる導線ができるうえ、ロック画面のプレビュー設定次第で本文が
隠れるため、通知に確認コードを載せても「押す直前に見えている」保証がありません。
通知の役目は気付かせて前面へ戻すことだけで、判断はこれまでどおり同意シートで
確認コードを照合しながら行います。

代償があります。

- **バッテリーを余分に消費します。** CPU はほぼ使いませんが、プロセスが
  眠らないぶん確実に減ります。既定でオフにしてあるのはこのためです。
- **電話や他アプリの排他再生で中断されると止まります。** 中断の終了通知を
  拾って鳴らし直しますが、中断中にサスペンドまで進むと通知自体が届かず、
  前面へ戻すまで止まったままになります。
- **音量ボタンが「メディア」を操作します。** 無音なので実害はありません。
- カテゴリは `.playback` + `.mixWithOthers` です。`.mixWithOthers` を外すと、
  他アプリで再生中の音楽を無音で上書きしてしまいます。
...
- **ファイルサイズが読めなかったことを握り潰さない。** `?? 0` で丸めると
  ...
  返してログに残します。
- **常駐の有無で「失敗」の意味が変わる。** 背面での listener / publish の
  切断は、通常はサスペンドの通知でしかないので障害ではありません。しかし
  無音再生で起きたままのときは本物の失敗です。`QuickShareAdvertiser` と
  `QuickShareBrowser` は `isForeground || staysAliveInBackground` (= `isAwake`)
  で判定し、ログのレベルと再試行の可否を切り替えます。実状態 (鳴っているか) を
  配ること。意図 (トグルの値) を配ると、中断で止まっているのに背面の再試行を
  延々と試み続けます。
