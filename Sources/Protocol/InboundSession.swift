...
/// 受け取る前に見せる 1 件ぶん。
struct IncomingItemPreview: Identifiable {
    /// `payload_id`。同じ転送の中で一意。
    let id: Int64
    let title: String
    let size: Int64
    let isText: Bool
...
final class InboundSession: ObservableObject {
    ...
    ///
    /// オフのときは `Documents/Received/` に残る。**既定はオフ。**
    /// 権限を要求する設定なので、オンにするのは操作した本人であるべき。
    ...
    /// `INTRODUCTION` で予告された、これから受け取るもの。
    ///
    /// 同意を求める時点では、ファイルは `files` に入るがテキストは
    /// `expectedTexts` にしか無い。同意画面が `files` だけを見ていたため、
    /// **テキストだけを受け取るときに「0 件」と出ていた**。
    /// 受け取る前に見せるものは、種類を問わずここに集める。
    @Published private(set) var incomingPreview: [IncomingItemPreview] = []
    ...
    init() {
        // どちらも既定はオフ。キーが無ければ false になる。
        //
        // 「写真」への保存を既定オンにしていたが、権限を要求する設定は
        // **こちらから勝手にオンにしない**のが筋。オンにする操作の時点で
        // 権限を訊き、拒否されたらオフに戻す、という流れに揃える。
        let defaults = UserDefaults.standard
        autoAcceptEnabled = defaults.bool(forKey: Self.autoAcceptDefaultsKey)
    ...
    private func handleIntroduction(_ introduction: Sharing_Nearby_IntroductionFrame) {
        ...
        var preview: [IncomingItemPreview] = newFiles.map {
            IncomingItemPreview(
                id: $0.payloadId, title: $0.displayPath,
                size: $0.totalSize, isText: false
            )
        }
        ...
        for meta in introduction.textMetadata {
            ...
            preview.append(IncomingItemPreview(
                id: meta.payloadID,
                title: meta.textTitle.isEmpty ? "(表題なし)" : meta.textTitle,
                size: meta.size,
                isText: true
            ))
