...
final class QuickShareAdvertiser: NSObject, ObservableObject {
    ...
    ///
    /// これが真なら、背面へ回っても listener と publish は落ちない。落ちたなら
    /// それは「サスペンドされた」ではなく本物の失敗なので、背面でも再試行するし
    ...
    private func handleListenerState(_ newState: NWListener.State) {
        ...
        case .failed(let error):
            ...
            // publish と同じ理由で、背面にいるあいだの失敗は障害ではない。
            // ただし無音再生で起きたままのときは本物の失敗。
            if isAwake {
                qlog(.error, "Advertiser: \(message)")
            } else {
                qlog(.info, "Advertiser: サスペンドで listener が切れました "
                    + "— \(describe(error))。前面復帰時に張り直します")
            }
            state = .failed(message)
    ...
    private func endpointInfoEncoded() -> String {
        // 自分の証明書があり、そう設定されていれば、広告の 16 バイトを
        // 証明書由来にする。相手はこれを復号して、こちらが誰かを特定できる。
        // 証明書を持たない相手には、ただの乱数と区別が付かない。
        let store = LocalCertificateStore.shared
        var metadata = EndpointInfo.randomMetadata()
        var hidden = false
        if store.useForAdvertisement,
           let certificate = store.active,
           let derived = certificate.advertisementMetadata() {
            metadata = derived
            hidden = store.hideNameInAdvertisement
        }
...
extension QuickShareAdvertiser: NetServiceDelegate {
    ...
    nonisolated func netService(_ sender: NetService, didNotPublish errorDict: [String: NSNumber]) {
        ...
        Task { @MainActor in
            // 背面にいるあいだの失敗は、iOS がサスペンド時に publish を
            // 落としたことの通知でしかない。前面に戻れば refresh が張り直すので
            // 障害ではない。本物の失敗と見分けが付くようにレベルを落とす。
            //
            // 失敗の配送は前面復帰の直前に来る。scenePhase が .active になる
            // 前なので、この時点の isForeground はまだ false になっている。
            ...
            if self.isAwake {
                ...
                qlog(.info, "Advertiser: サスペンドで publish が切れました "
                    + "— \(name) (\(code))。前面復帰時に張り直します")
            }
