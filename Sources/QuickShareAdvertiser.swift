...
final class QuickShareAdvertiser: NSObject, ObservableObject {
    ...
    /// 相手に見せる名前。`DeviceNameStore` が持つ値をそのまま使う。
    ///
    /// 広告は起動時に組み立てるので、名前を変えたら張り直す必要がある。
    ...
    /// 名前を変えたときなど、健全でも張り直したい場面のため。
    ///
    /// `refresh(reason:)` は「壊れていたら直す」ものなので、
    /// 動いている広告を作り直すには使えない。
    func restart(reason: String) {
        guard isEnabled else { return }
        qlog(.info, "Advertiser: \(reason)ため張り直します")
        endAdvertising()
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
            guard let self, self.isEnabled else { return }
            self.beginAdvertising()
...
/// OS が返す端末名。
///
/// iOS 16 以降は「iPad」のような機種名に丸められる。実際の名前を出したいときは
/// `DeviceNameStore` の値を使う。
