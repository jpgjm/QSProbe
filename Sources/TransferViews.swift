...
struct ConsentSheet: View {
    ...
    var body: some View {
        ...
        // 開いた時点で広げておく。
        //
        // 既定の `.medium` だと、受け取るものの一覧が下に隠れる。
        // 何を受け取るか分からないまま「受け取る」を押させることになり、
        // 確認の意味が薄れる。縮めたい人は下げられるよう `.medium` も残す。
    ...
    private var itemsSection: some View {
        Section {
            if session.incomingPreview.isEmpty {
                Text("相手はまだ中身を知らせていません。")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
            } else {
                ForEach(session.incomingPreview) { item in
                    HStack(spacing: 8) {
                        Image(systemName: item.isText ? "text.alignleft" : "doc")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Text(item.title)
                            .font(.callout)
                            .lineLimit(2)
                        Spacer()
                        Text(ByteFormat.short(item.size))
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
            ...
            Text("受け取るもの (\(session.incomingPreview.count) 件)")
        }
    }
