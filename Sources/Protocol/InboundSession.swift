...
final class InboundSession: ObservableObject {
    ...
    /// 相手の署名を検証できたか。`PAIRED_KEY_RESULT` の返し方が変わる。
    @Published private(set) var peerVerified = false

    /// 受信した写真・動画を「写真」App に入れるか。
    ///
    /// オフのときは `Documents/Received/` に残る。既定はオン。
    @Published var saveMediaToPhotos: Bool {
        didSet {
            UserDefaults.standard.set(saveMediaToPhotos, forKey: Self.saveMediaDefaultsKey)
            qlog(.info, "Session: 写真・動画の保存先を "
                + (saveMediaToPhotos ? "「写真」App" : "ファイル") + " にしました")
        }
    }

    private static let autoAcceptDefaultsKey = "QSProbe.autoAcceptEnabled"
    private static let saveMediaDefaultsKey = "QSProbe.saveMediaToPhotos"
    ...
    private var expectedTexts: [Int64: (title: String, kind: ReceivedTextKind)] = [:]
    ...
    init() {
        ...
        // こちらは既定オン。未設定と「明示的にオフ」を区別する必要がある。
        let defaults = UserDefaults.standard
        if defaults.object(forKey: Self.saveMediaDefaultsKey) == nil {
            saveMediaToPhotos = true
        } else {
            saveMediaToPhotos = defaults.bool(forKey: Self.saveMediaDefaultsKey)
        }
    }
    ...
    func cancel() {
        stopKeepAlive()
        TransferNotification.clearConsentRequired()
    ...
    /// もう転送を抱えていない状態か。
    /// ここに入ったあとに届く接続エラーは、後始末の副産物でしかない。
    private var isTerminal: Bool {
        switch stage {
        case .completed, .rejected, .failed, .closed, .idle:
            return true
        default:
            return false
        }
    ...
    private func handleConnectionState(_ state: NWConnection.State) {
        ...
        case .failed(let error):
            // 受信が終わったあとに届く failed は後始末の副産物。障害ではない。
            if isTerminal {
                qlog(.info, "Session: 後始末中に接続が切れました — \(error)")
                return
            }
    ...
    ///
    /// 併せてバックグラウンド継続タスクの開始 / 終了もここで行う。
    /// このメソッドが呼ばれる箇所は「転送を抱えている区間」とちょうど
    /// 一致しているので、別に呼び出し点を作らず相乗りさせている。
    private func setIdleTimer(disabled: Bool) {
        ...
        if disabled {
            BackgroundTransferTask.shared.begin(
                role: .receiving,
                detail: peerDeviceName ?? "受信を開始しています"
            ) { [weak self] in
                self?.cancel()
            }
            publishBackgroundProgress()
        } else {
            BackgroundTransferTask.shared.end(role: .receiving)
        }
    }

    /// システムの進捗 UI (ロック画面のバナー) に今の状態を渡す。
    ///
    /// 表示は転送全体の合計で出す。ファイルごとに出し分けると、
    /// 複数件のときにバナーの数字が行ったり来たりして読めない。
    private func publishBackgroundProgress() {
        let total = files.reduce(Int64(0)) { $0 + max($1.totalSize, 0) }
        let done = files.reduce(Int64(0)) { $0 + $1.receivedBytes }

        guard total > 0 else {
            BackgroundTransferTask.shared.update(
                role: .receiving,
                detail: peerDeviceName ?? "受信を開始しています",
                percent: -1
            )
        ...
        BackgroundTransferTask.shared.update(
            role: .receiving,
            detail: "\(percent)% · \(ByteFormat.short(done)) / \(ByteFormat.short(total))",
            percent: percent
        )
    }
    ...
    /// 背面で同意待ちに入ったことを知らせる。
    ///
    /// 通知にはボタンを付けない。承諾/拒否は前面の同意シートで、PIN を
    /// 見ながら判断する。通知の役目は気付かせて前面へ戻すことだけ。
    ///
    /// なお、この通知が意味を持つのは無音再生の常駐がオンのときだけ。
    /// 常駐オフなら iOS がプロセスを眠らせるので、そもそも背面で
    /// INTRODUCTION を受け取れない。
    private func requestConsentNotification() {
        let itemCount = files.count + expectedTexts.count
        TransferNotification.notifyConsentRequired(
    ...
    private func handleConnectionRequest(_ payload: Data, index: Int) throws {
        ...
        if let info = EndpointInfo.parse(request.endpointInfo) {
            ...
            detail = "\(name) / \(detail)"
            identifyPeerCertificate(metadata: info.metadata)
        }
    ...
    private func handlePlaintextConnectionResponse(_ payload: Data, index: Int) throws {
        ...
        if AccountStore.shared.isEnabled,
           ...
           let signature = certificate.sign(authToken: token, isSender: false) {
            ...
            qlog(.ok, "Session:   ★ 自分の証明書で署名しました (受信側の印)")
        }
    ...
    /// 受信し終えたファイルの置き場所を決める。
    ///
    /// 写真・動画で、かつトグルがオンなら「写真」App へ移す。
    /// それ以外や失敗時は `Documents/Received/` に残す。
    private func storeReceivedFile(_ file: ReceivingFile) {
        guard saveMediaToPhotos, PhotoLibrarySaver.isMedia(file.destinationURL) else {
            qlog(.info, "Session:     保存先 = Received/\(file.destinationURL.lastPathComponent)")
            return
        }

        PhotoLibrarySaver.save(file.destinationURL) { [weak self] outcome in
            guard let self else { return }
            switch outcome {
            case .savedToPhotos:
                file.markSavedToPhotos()
                self.progressTick &+= 1
                qlog(.ok, "Session:     保存先 = 「写真」App — \(file.name)")
            case .notMedia:
                qlog(.info, "Session:     保存先 = Received/\(file.destinationURL.lastPathComponent)")
            case .keptInFiles(let reason):
                qlog(.warn, "Session:     「写真」に入れられませんでした — \(reason)")
                qlog(.info, "Session:     保存先 = Received/\(file.destinationURL.lastPathComponent)")
            }
        }
    }
    ...
    private func checkAllComplete() {
        ...
        // バックグラウンドで終わったときは画面に何も残らないので通知を出す。
        // 前面にいるあいだは iOS 側が握り潰すため、邪魔にはならない。
        TransferNotification.notifyCompleted(
            title: "受信が完了しました",
            body: TransferNotification.summary(
                names: files.map { $0.displayPath },
                totalBytes: files.reduce(Int64(0)) { $0 + max($1.totalSize, 0) }
            )
        )
    ...
    private func handleBytesChunk(
        ...
        if let expected = expectedTexts.removeValue(forKey: header.id) {
            ...
            texts.append(received)
            let title = expected.title
            qlog(.ok, "Session:   ★★★ テキスト受信完了 — \(title) (\(buffer.count) バイト)")
            if received.openableURL != nil {
                qlog(.info, "Session:     URL として開けます")
            }
    ...
    private func describeTransferSetupFrame(_ data: Data, index: Int, byteCount: Int) {
        ...
        case .pairedKeyResult:
            let status = frame.v1.pairedKeyResult.status
            detail = "PAIRED_KEY_RESULT / status = \(status)"
            // 相手がこちらをどう判定したか。転送が始まらないときの切り分けに使う。
            switch status {
            case .success:
                qlog(.ok, "Session:   ★ 相手がこちらを検証しました (success)")
            case .fail:
                qlog(.warn, "Session:   ⚠ 相手の検証に失敗しました (fail)")
                qlog(.info, "Session:     こちらの署名か、広告に載せた 16 バイトが"
                    + "噛み合っていません")
            default:
                break
            }
        ...
        case .introduction:
            ...
            let names = files.map { $0.displayPath }
                + texts.map { $0.title }
                + expectedTexts.values.map { $0.title }
    ...
    private func handleIntroduction(_ introduction: Sharing_Nearby_IntroductionFrame) {
        ...
        for meta in introduction.textMetadata {
            let kind = ReceivedTextKind(meta.type)
            expectedTexts[meta.payloadID] = (title: meta.textTitle, kind: kind)
            qlog(.ok, "Session:     - テキスト: \(meta.textTitle) "
                + "(\(meta.size) バイト, type=\(meta.type))")
        }
        files = newFiles
        ...
        guard autoAcceptEnabled else {
            ...
            requestConsentNotification()
            return
        }

        // 本人確認できた相手だけに絞る設定。
        //
        // 確認コードは「番号を読める距離に居る」ことを示すだけで、
        // 誰が送ってきたかは示さない。署名の検証は接続ごとに新しい
        // authString を対象にするので、広告を丸ごとコピーして名乗る手
        // (リプレイ) では通らない。**自動承認の相手を絞る根拠として
        // 使えるのは、この検証だけ。**
        let account = AccountStore.shared
        if account.isEnabled, account.autoAcceptVerifiedOnly, !peerVerified {
            qlog(.warn, "Session: 自動承認の対象外です (本人確認できていません)")
            qlog(.ok, "Session: ★★★ 同意待ちです。PIN を確認して「受け入れる」を押してください")
            requestConsentNotification()
            return
        }
        ...
        if peerVerified {
            qlog(.ok, "Session: ★ 本人確認できた相手なので、確認なしで受け入れます")
        } else {
    ...
    /// `signed_data` を、特定した証明書の公開鍵で検証する。
    ///
    /// `secret_id_hash` の一致は「同じ鍵を持っている」ことの弱い証拠でしかない。
    /// **署名が通れば、相手が本人であることが直接証明できる。** 第 4 段の本丸。
    ///
    /// いまは記録するだけで、`PAIRED_KEY_RESULT` の返し方は変えない。
    /// 何に署名しているかが確定してから切り替える。
    private func verifySignedData(_ signature: Data) {
        ...
        guard let certificate = identifiedCertificate else {
            qlog(.info, "Session:   証明書を特定できていないので、署名は検証できません")
            return
        }
        guard let token = ukey2AuthKey else {
            qlog(.warn, "Session:   authString が無いので、署名を検証できません")
            return
        }
        ...
        if let recipe = PairedKeyVerifier.verify(
            ...
            peerVerified = true
            qlog(.ok, "Session:   ★★★★ 署名を検証できました (\(recipe))")
            qlog(.ok, "Session:     相手は証明書の持ち主本人です")
        } else {
            qlog(.warn, "Session:   署名を検証できませんでした")
            qlog(.info, "Session:     公開鍵 = \(certificate.publicKey.count) バイト / "
                + "署名 = \(signature.count) バイト / authString = \(token.count) バイト")
            qlog(.info, "Session:     authString 先頭 = \(Self.hex(token, limit: 8))")
            qlog(.info, "Session:     → 署名対象が違います。相手の画面に出る確認番号が "
                + "\(pinCode ?? "-") と一致するかを見てください")
            qlog(.info, "Session:       食い違うなら authString 自体が別物です")
        }
    }

    /// 広告に載っている 16 バイトから、相手の証明書を割り出す。
    ///
    /// `secret_id_hash` の照合と違い、こちらは**正誤がはっきりする**。
    /// 復号した metadata 鍵を `HMAC-SHA256(空, 鍵)` にかけた値が、証明書の
    /// `metadata_encryption_key_tag` と一致するかで判定できるため。
    /// tag の作り方は 34/34 件で確認済み。
    ///
    /// 当たれば「相手の証明書を手元に持っている」ことの証明になる。
    /// 外れれば、持っている証明書が相手のものではないと分かる。
    private func identifyPeerCertificate(metadata: Data) {
        ...
        guard metadata.count >= 16 else {
            qlog(.info, "Session:   広告の metadata が \(metadata.count) バイトで、"
                + "証明書の照合には足りません")
            return
        }

        let pool = store.certificates.map { $0.certificate }
        if let match = AdvertisementIdentity.identify(metadata: metadata, certificates: pool) {
            identifiedCertificate = match.certificate
            qlog(.ok, "Session:   ★★★ 相手の証明書を特定しました")
            if let metadata = CertificateMetadata.decrypt(match.certificate) {
                qlog(.ok, "Session:   持ち主 = \(metadata.summary)")
            }
            qlog(.ok, "Session:     secret_id = "
                + "\(CertificateStore.hex(match.certificate.secretId, limit: 8))")
            qlog(.info, "Session:     metadata 鍵 = "
                + "\(CertificateStore.hex(match.metadataKey))")
            qlog(.info, "Session:     → この鍵で encrypted_metadata を開けば端末名が出ます")
        } else {
            qlog(.warn, "Session:   広告から証明書を特定できませんでした "
                + "(\(store.certificates.count) 件と照合)")
            qlog(.info, "Session:     salt = \(Self.hex(metadata.prefix(2)))"
                + " / 暗号化鍵 = \(Self.hex(metadata.dropFirst(2).prefix(14)))")
        }
    }

    /// アカウント連携で取ってきた証明書と突き合わせる。**記録するだけ。**
    ///
    /// 一致しても `PAIRED_KEY_RESULT` の返し方は変えない。まず
    /// 「`secret_id_hash` がどう作られているか」を確定させるのが先で、
    /// 署名の検証はそのあと。実験機能がオフなら何もしない。
    /// `secret_id_hash` から相手の証明書を割り出す。
    ///
    /// 広告からの特定 (`identifyPeerCertificate`) と役割は同じだが、
    /// **材料が違う**。広告は接続の入口でしか見られず、相手が新しい証明書に
    /// 切り替えた直後などは手元の広告と噛み合わないことがある。
    ///
    ...
    private func lookUpCertificate(for hash: Data) {
        ...
        if let stored = store.match(secretIdHash: hash, authToken: token) {
            qlog(.ok, "Session:   ★ 相手はこちらを認識しています")
            qlog(.info, "Session:     相手が使ったこちらの証明書 = \(stored.summary)")
        } else {
            qlog(.info, "Session:   相手はこちらを認識していません (乱数を送っています)")
            qlog(.info, "Session:     相手がまだこちらの証明書を取得していない可能性")
        }
    }
    ...
    /// `PAIRED_KEY_RESULT` を返す。
    ///
    /// 相手の署名を証明書の公開鍵で検証できていれば `success`、
    /// そうでなければ従来どおり `unable`。
    ///
    /// `success` は「相手が名乗ったとおりの人物だと確認できた」という意味で、
    /// 検証を通していないのに返してはいけない。逆に、検証できたのに
    /// `unable` を返し続けるのは、この段の成果を捨てているのと同じ。
    private func sendPairedKeyResult() {
        ...
        let label = peerVerified ? "PAIRED_KEY_RESULT (success)" : "PAIRED_KEY_RESULT (unable)"
        try? sendTransferSetupFrame(result, label: label)
    }
