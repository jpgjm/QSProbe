...
struct ContentView: View {
    ...
    // バックグラウンド常駐 (無音再生)。バッテリーを食うので既定でオフ。
    @AppStorage("QSProbe.backgroundKeepAlive") private var backgroundKeepAlive = false
    ...
    var body: some View {
        ...
        .onChange(of: keepAlive.state) { _ in
            // 無音再生が中断されたら「起きたまま」判定も下ろす。
            applyKeepAliveState()
        }
    ...
    /// 共有シートの拡張から起こされたときに呼ばれる。
    ///
    /// URL には情報を載せていない。「起きて受信箱を見ろ」という合図だけなので、
    /// ここでやるのは取り込みの実行だけ。
    private func handleOpenURL(_ url: URL) {
        guard QSProbeURLScheme.isShareCallback(url) else {
            qlog(.info, "知らない URL で起動されました — \(url)")
            return
        }
        drainShareInbox(source: "共有シート")
    }

    /// 共有コンテナに溜まっているぶんを送信候補へ移す。
    private func drainShareInbox(source: String) {
        let items = ShareInbox.drain()
        guard !items.isEmpty else { return }
        addPendingItems(items, source: source)
    }
    ...
    private func handleScenePhase(_ phase: ScenePhase) {
        ...
        case .active:
            ...
            // 共有シートからの取り込みはここでも試す。
            //
            // 拡張がホストを起こす openURL は、拡張の実装や iOS の版に
            // 左右されて失敗しうる。一方、共有シートから QSProbe を選べば
            // どのみち前面には出てくるので、ここで受信箱を見れば
            // openURL が不発でも取りこぼさない。
            // 溜まっていなければ何もしないので、毎回呼んで構わない。
            drainShareInbox(source: "共有シート")
    ...
    private func setUpOnce() {
        ...
        AppGroupProbe.dumpToLog()
        Outbox.purge()

        // 完了通知の権限はここで一度だけ求める。
        // 転送の開始時に訊くと、接続や相手の同意待ちとダイアログが重なって
        // 操作の邪魔になる。起動時に済ませておけば転送中は出てこない。
        TransferNotification.prepare()

        // アプリが落ちている間に共有されたぶんを拾う。
        // Outbox.purge() の後でなければ、取り込んだ直後に消されてしまう。
        drainShareInbox(source: "共有シート (起動時)")
        ...
        if backgroundKeepAlive {
            qlog(.info, "前回の設定を復元: バックグラウンド常駐をオンにします")
        }
        syncKeepAlive()
        if browseEnabled {
    ...
    private var receiveSection: some View {
        Section {
            ...
            Toggle("画面を閉じても待ち受ける", isOn: backgroundKeepAliveBinding)

            if backgroundKeepAlive {
                switch keepAlive.state {
                case .running:
                    statusRow(title: "常駐", value: "無音を再生中", color: .green)
                case .interrupted(let reason):
                    statusRow(title: "常駐", value: "中断中 (\(reason))", color: .orange)
                case .failed(let message):
                    statusRow(title: "常駐", value: "失敗: \(message)", color: .red)
                case .stopped:
                    statusRow(
                        title: "常駐",
                        value: advertiser.isEnabled ? "停止中" : "受信がオフのため待機",
                        color: .secondary
                    )
                }

                Text("無音を鳴らし続けてアプリを眠らせないことで、ホームに戻っても"
                     + "受け取れるようにします。バッテリーを余分に消費します。"
                     + "電話や他アプリの排他再生で中断されると止まります。")
                    .font(.caption2)
    ...
    private var sendSection: some View {
        Section {
            HStack(spacing: 10) {
                pickerButton("ファイル・フォルダ", systemImage: "doc") {
    ...
    private var advertisingBinding: Binding<Bool> {
        Binding(
            ...
            set: { isOn in
                ...
                syncKeepAlive(receive: isOn)
            }
        )
    }

    private var backgroundKeepAliveBinding: Binding<Bool> {
        Binding(
            get: { backgroundKeepAlive },
            set: { isOn in
                backgroundKeepAlive = isOn
                syncKeepAlive(keep: isOn)
            }
        )
    }

    /// 「受信オン」かつ「常駐オン」のときだけ無音を鳴らす。
    ///
    /// 受信がオフなら待ち受けるものが無いので、鳴らす意味も無い。
    /// 広告側にも実状態を伝えて、背面での再試行を許すかどうかを切り替える。
    ///
    /// トグルの `set` から呼ぶときは、書き込んだ値をそのまま渡すこと。
    /// `@AppStorage` はビューの再評価で反映されるので、同じクロージャの中で
    /// 読み直すと古い値が返りうる。
    private func syncKeepAlive(receive: Bool? = nil, keep: Bool? = nil) {
        let wanted = (receive ?? advertiser.isEnabled) && (keep ?? backgroundKeepAlive)
        keepAlive.setEnabled(wanted)
        applyKeepAliveState()
    }

    /// 無音再生が実際に鳴っているかを、広告と探索の両方へ伝える。
    ///
    /// 意図 (`backgroundKeepAlive`) ではなく実状態を配ること。中断で止まって
    /// いるのに真のままだと、サスペンドされる状況で背面の再試行を延々と
    /// 試みることになる。
    private func applyKeepAliveState() {
        let awake = keepAlive.isRunning
        advertiser.staysAliveInBackground = awake
        browser.staysAliveInBackground = awake
    }
    ...
    /// ファイルとフォルダをまとめて受け取る。
    ///
    /// `.item` と `.folder` を同時に許可したので、返ってくる URL には
    /// ファイルとフォルダが混ざる。1 件ずつ見て振り分ける。
    ///
    /// フォルダは走査に時間がかかるためバックグラウンドへ回す。ファイルは
    /// すぐ確定するので、先にまとめて一覧へ入れて反応を返す。
    private func handlePick(_ result: Result<[URL], Error>) {
        ...
        case .failure(let error):
            qlog(.warn, "選択に失敗: \(error)")
        }
    }

    /// フォルダかどうか。
    ///
    /// `resourceValues` はセキュリティスコープの中でないと失敗することがあるので、
    /// まず `hasDirectoryPath` を見る。ドキュメントピッカーが返すフォルダの URL は
    /// 末尾がスラッシュになるため、これだけでほぼ確定する。
    /// 念のため、判定できなかったときだけスコープを開いて問い合わせる。
    private func isDirectory(_ url: URL) -> Bool {
        if url.hasDirectoryPath { return true }

        // SecurityScope は deinit で閉じるので、問い合わせが終わるまで生かしておく。
