...
final class OutboundSession: ObservableObject {
    ...
    /// 受信側と同じ理由で、この呼び出し点が転送区間とちょうど一致している。
    private func setIdleTimer(disabled: Bool) {
        ...
        if disabled {
            BackgroundTransferTask.shared.begin(
                role: .sending,
                detail: targetName.map { "\($0) へ接続しています" } ?? "接続しています"
            ) { [weak self] in
                self?.cancel()
            }
        } else {
            BackgroundTransferTask.shared.end(role: .sending)
        }
    }

    /// システムの進捗 UI (ロック画面のバナー) に今の状態を渡す。
    ///
    /// 送信は「相手の同意待ち」で長く止まることがある。その区間は
    /// 進捗を出しようがないので、状態の文言だけを不定表示で見せる。
    private func publishBackgroundProgress() {
        guard stage == .sending else {
        ...
        let done = files.reduce(Int64(0)) { $0 + $1.sentBytes }

        guard total > 0 else {
            BackgroundTransferTask.shared.update(
                role: .sending,
                detail: targetName ?? stage.rawValue,
                percent: -1
            )
            return
        }
    ...
    private func handleConnectionState(_ state: NWConnection.State) {
        ...
        case .failed(let error):
            // 転送が終わったあとに届く failed は、後始末済みの接続が
            // サスペンドや相手側の切断で落ちただけ。障害ではない。
            //
            // finishAndDisconnect() は cancel() を 0.5 秒後に回すので、
            // その前後に相手の RST が届くとここに来る。転送自体は
            // completed になっているので、赤いエラーを出すと紛らわしい。
            if isTerminal {
                qlog(.info, "Outbound: 後始末中に接続が切れました — \(error)")
                return
            }
